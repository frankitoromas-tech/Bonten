...
export default function ComunidadContent() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const savedAlias = localStorage.getItem('bonten:member_alias');
      if (savedAlias) {
        setMemberAlias(savedAlias);
        setHasPledged(true);
      } else {
    ...
    <div className="container mx-auto px-4 py-10 max-w-6xl space-y-10">
      ...
      >
        ...
        <div className="relative z-10 pt-6 flex flex-wrap gap-3">
          ...
          {memberAlias ? (
            <div className="px-4 py-2 rounded-xl bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 font-mono text-xs flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span>Miembro Activo: <strong>@{memberAlias}</strong></span>
            </div>
          ) : (
            <Link
              href="/auth/register"
